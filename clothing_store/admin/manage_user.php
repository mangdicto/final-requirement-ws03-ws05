...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Poppins', sans-serif; }
        ...
        /* Action Buttons */
        .action-link { text-decoration: none; font-weight: 600; font-size: 0.85rem; padding: 5px 10px; border-radius: 5px; transition: 0.2s; display: inline-block; margin: 0 2px; }
        .btn-archive { color: var(--danger); border: 1px solid var(--danger); }
        .btn-archive:hover { background: var(--danger); color: white; }
        .btn-reset { color: var(--accent-color); border: 1px solid var(--accent-color); }
                ...
                <tbody>
                    ...
                    if($result && $result->num_rows > 0) {
                        while($row = $result->fetch_assoc()) {
                            ...
                            $archive_url = "user_action.php?id=" . $id . "&action=archive&csrf_token=" . $_SESSION['csrf_token'];
                            $reset_url = "reset_user_password.php?id=" . $id . "&csrf_token=" . $_SESSION['csrf_token'];
                            ?>
                            <tr>
                                ...
                                <td class="action-cell">
                                    <a href="<?php echo $archive_url; ?>" class="action-link btn-archive" onclick="return confirm('Archive this user?')">Archive</a>
                                    <a href="<?php echo $reset_url; ?>" class="action-link btn-reset">Reset Password</a>
                                </td>
                            </tr>
                            <?php
                        }
                ...
                <tbody>
                    ...
                    if($archived_result && $archived_result->num_rows > 0) {
                        while($row = $archived_result->fetch_assoc()) {
                            $id = base64_encode($row['id']);
                            $restore_url = "user_action.php?id=" . $id . "&action=restore&csrf_token=" . $_SESSION['csrf_token'];
                            ?>
                            <tr>
                                <td><?php echo htmlspecialchars($row['first_name']); ?></td>
                                <td><?php echo htmlspecialchars($row['last_name']); ?></td>
                                ...
                                <td class="action-cell">
                                    <a href="<?php echo $restore_url; ?>" class="action-link btn-restore" onclick="return confirm('Restore this user?')">Restore</a>
                                </td>
                            </tr>
                            <?php
                        }
